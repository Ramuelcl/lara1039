...
class BancaController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        foreach ($archivos as $archivo) {
            ...
            if ($this->checkFileImported($nombreOriginal)) {
                ...
                $camposArchivo = ['Date', 'Libelle', 'MontantEUROS', 'MontantFRANCS'];
    ...
    public function eliminarRegistrosDuplicados()
    {
        ...
        return redirect()
            ->back()
            ->with('success', 'Registros duplicados eliminados correctamente');
    }
    ...
    public function TraspasoAMovimientos()
    {
        ...
        try {
            ...
            if ($registros) {
                ...
                return redirect()
                    ->back()
                    ->with('success', 'No hay registros a mover');
            }
        } catch (\Throwable $e) {
            // Manejar el error
            // Puedes registrar el error, mostrar un mensaje o realizar alguna otra acción según tus necesidades
        ...
        return redirect()
            ->back()
            ->with('success', 'Registros movidos correctamente');
    }
        ...
        foreach ($archivos as $archivo) {
            ...
            if ($this->checkFileImported($nombreOriginal)) {
                ...
                try {
                    ...
                    return redirect()
                        ->back()
                        ->with('success', 'El archivo se ha importado correctamente.');
                } catch (\Exception $e) {
                    // dd($archivo, $e->getMessage());
                    return redirect()
                        ->back()
                        ->with('error', 'Ha ocurrido un error al importar el archivo: ' . $e->getMessage());
                }
            }
        }
    }
